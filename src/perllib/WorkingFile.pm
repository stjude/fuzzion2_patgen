...
sub outfile_setup {
  ...
  if (my $type = $self->compress()) {
    if ($type eq "bz2") {
      ...
      $complete_name .= ".bgz" unless $complete_name =~ /\.b?gz$/i;
    } else {
...
sub output_filehandle {
  ...
  unless ($fh) {
	...
	$cmd = sprintf "|bgzip -l%d >%s", $level, $fn;
	# "-l" compression-level option requires a newer htslib,
	# tested with htslib/1.10.2
