...
#    printf STDERR "range: %s (raw=%s) %s %s %d-%d\n", $chr, $chr_raw, $row->{name}, $row->{name2}, $row->{txStart}, $row->{txEnd};
	...
	die "coding prefix not defined for prefix $prefix in $refseq" unless defined $is_coding;
      } else {
	# 10/2020: CICERO's refFlat file may now have non-refSeq records
	# appended from e.g. ENSEMBL
	printf STDERR "WARNING: accession %s not in refSeq format, assuming coding\n", $refseq;
	$is_coding = 1;
	# e.g. DUX4 / ENST00000563716.5
      }
...
#      next if $hit->{txStart} > $end;
    ...
    printf STDERR "can't find annotations for $chr, ref=$ref_name\n";
    # OK for MT
  }
...
sub results_strand {
  my ($self) = @_;
  my $rr = $self->results_rows();
  my %strand = map {$_->{strand}, 1} @{$rr};
  my $result;
  if (keys %strand == 1) {
    $result = (keys %strand)[0];
  }
}
