...
@FAI::EXPORT_OK = qw(get_fai build_fai);
...
sub setup {
  ...
  # TO DO: auto-call build_fai()?
  # OTOH: directory may not be writable, e.g. for TARTAn files
  open(FAI, $fai) || die;
...
sub build_fai {
  # create .fai index file from FASTA
  my ($f_fa) = @_;
  my $f_fai = $f_fa . ".fai";
  ...
  my $seq = "";
  my $seq_start;
  my ($raw_line_length, $cooked_line_length);
  my $lines_in_sequence;

  my @queue;
  my $longest_raw_line;
  my $longest_cooked_line;
  my $flush = sub {
    die unless $id and $seq;
#    printf $fh "%s\n", join "\t", $id, length($seq), $seq_start, $raw_line_length, $cooked_line_length;
    push @queue, [ $id, length($seq), $seq_start, $longest_cooked_line, $longest_raw_line ];
  };

  open(IN, $f_fa) || die "can't open $f_fa";
  ...
  while (<IN>) {
    if (/^>/) {
      ...
      $lines_in_sequence = 0;
      $longest_raw_line = 0;
      $longest_cooked_line = 0;
      /^>(\S+)/ || die;
      $id = $1;
    } else {
#      if ($lines_in_sequence++ > 1) {
	# sequence contains multiple lines, meaning that the previous
	# line was a full line.  Save details of long line lengths.
#
      # abandoned: if early sequences are short, results will be
      # different than produced by samtools, which seems to use the
      # longest line

      $raw_line_length = length $_;
      chomp;
      $cooked_line_length = length $_;
      $seq .= $_;

      $longest_raw_line = $raw_line_length if $raw_line_length > $longest_raw_line;
      $longest_cooked_line = $cooked_line_length if $cooked_line_length > $longest_cooked_line;
  ...
  # write .fai file:
  my $wf = new WorkingFile($f_fai);
  my $fh = $wf->output_filehandle();
  foreach my $record (@queue) {
    die "record must be exactly 5 fields" unless @{$record} == 5;
    printf $fh "%s\n", join "\t", @{$record};
  }
  $wf->finish();
}
...
sub get_flanking {
  # return sequence flanking a position, by strand and direction
  my ($self, %options) = @_;
  my $direction = $options{"-direction"} || die;
  my $chr = $options{"-chr"} || die "-chr";
  my $pos = $options{"-pos"} || die "-pos";
  my $ort = $options{"-ort"} || die "-ort";
  my $length = $options{"-length"} || die "-length";

  my $result;
  if ($direction eq "downstream") {
    my $max_len_i = $length - 1;
    # TO DO: centralize this code chunk
    if ($ort eq "+") {
    ...
    die "only downstream implemented so far";
  }
  return $result;
}
