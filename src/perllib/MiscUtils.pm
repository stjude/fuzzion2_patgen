...
sub unique_ordered_list {
  ...
  my $values;
  if (my $key = $options{"-key"}) {
    $values = [ map {$_->{$key}} @{$array_in} ];
  } else {
    $values = $array_in;
  }

  foreach my $thing (@{$values}) {
    next if $saw{$thing};
    $saw{$thing} = 1;
    push @out, $thing;
  }
...
sub float_trim {
  ...
  if ($v_out =~ /^(\d\.\d+)e([\+\-]\d\d$)/) {
    #
    # convert exponential notation to standard float, if advantageous:
    #
    my ($v, $exp) = ($1, $2);
    if ($v == 0 and $exp == 0) {
      # 0.0000e+00 is 0
      ...
      # can losslessly convert to standard float formatting,
      # then possibly trim further, e.g.
      # 9.9919e-01 (10 characters) ultimately becomes .99919 (6 characters)
      $v_out = sprintf "%f", $v_in;
    } elsif ($exp >= -4) {
      # -4 requires deeper sprintf formatting
      # 1.0547e-04 => 0.0001054700 => .00010547 (9 characters)
      # can't save space with -5 and below:
      # 1.0547e-05 => 0.0000105470 => .000010547 (10 characters,
      # no advantage vs. exponential notation)
      $v_out = sprintf "%.10f", $v_in;
    }
  }
...
#      print STDERR "tail $v_in $float\n";
    ...
    $v_out = $sign . $float;
    # TO DO: optionally trim + sign?  (+ is expected default behavior)
...
sub field_name_dwim {
  # "do what I mean" for field/column names, adjusting for capitalization
  my (%options) = @_;
  my $h_actual = $options{"-actual"} || die "-actual";
  my $h_query = $options{"-query"} || die "-query";

  my %header_map;
  foreach my $h (@{$h_actual}) {
    # headers actually present
    my $lc = lc($h);
    $header_map{$lc} = $h;
  }

  foreach my $hr (@{$h_query}) {
    # headers we're looking for
    # references to variables containing field names
    my $label = $$hr;
    my $lc = lc($label);
    if (my $actual = $header_map{$lc}) {
      if ($label ne $actual) {
	printf STDERR "field %s: using %s\n", $label, $actual;
	$$hr = $actual;
      }
    } else {
      die "can't find column named $label";
    }
  }
