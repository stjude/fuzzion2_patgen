...
sub setup {
  ...
  # may be optionally provided for headerless files
  unless ($headers) {
    $headers = $self->next_row();
...
sub write_init {
  ...
  if (my $he = $self->headers_extra) {
    if ($self->extra_prepend) {
      unshift @headers, @{$he};
    } else {
      push @headers, @{$he};
    }
...
sub write_row {
  ...
  my @extra;
  my $extra_prepend = $self->extra_prepend();
  ...
  if (my $hiw = $self->header_indices_wanted) {
    die "header subset with prepend not implemented" if $extra_prepend;
    printf $fh "%s\n", join $self->delimiter(), @{$line}[@{$hiw}], @extra;
  } elsif (@extra) {
    if ($extra_prepend) {
      printf $fh "%s\n", join $self->delimiter(), @extra, @{$line};
    } else {
  ...
  # the following code is cleaner but quite a bit slower:
  # my @out;
  # if (my $hiw = $self->header_indices_wanted) {
  #   @out = @{$line}[@{$hiw}];
  # } else {
...
sub get_query_row {
  # separate function so as not to slow down iterator version
  my ($self, $row) = @_;
  return [ @{$row}[@{$self->query_indexes()}] ];
}
