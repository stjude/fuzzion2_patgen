...
sub header_setup {
  ...
  if ($self->headers == 3) {
    ...
    @headers = @{$headers_raw};
  } elsif (ref $self->headers() eq "CODE") {
    # file is headered, but header requires preprocessing
    # (e.g. fuzzion2 "fuzzall" report has non-unique headers)
    my $callback = $self->headers();
    my @h = $self->next();
    # file's raw header line (ignored)
    &$callback(\@h);
    # external code tweaks heaaders
    $self->headers_raw(\@h);
    @headers = @h;
  } else {
    # autodetect
