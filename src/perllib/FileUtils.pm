...
sub find_binary {
  ...
  $bin2module{"blastn"} = "blast";
  $bin2module{"blastp"} = "blast";
  $bin2module{"gfClient"} = "blat/3.5";
  # HPCF doesn't have gfClient installed for new default blat/3.6

#  $bin2module{"tabix"} = "tabix";
#  $bin2module{"bgzip"} = "tabix";
  $bin2module{"tabix"} = "htslib";
  $bin2module{"bgzip"} = "htslib/1.15.1";
  # htslib has newer version; 1.15.1+ required for compression level
...
sub sort_delimited_file {
  ...
  if ($compress) {
    open(SDFTMP_OUT, "| gzip >" . $outfile_tmp) || die;
  } else {
  ...
  #
  my $fields;
  my $f = $options{"-field"} || die "-field";
  my $field_options = $options{"-field-options"} || [];
  if (ref $f) {
    # multiple fields
    $fields = $f;
  } else {
    $fields = [ $f ];
    # single field (original design)
  }

  my @sort_cmd;

  my $sort_cmd = sprintf '|sort -t "%s"', $delimiter;
  for (my $i = 0; $i < @{$fields}; $i++) {
    my $field = $fields->[$i];
    my $field_number = $h2i{$field} || die "can't find header entry for $field";
    $sort_cmd .= sprintf ' -k%d,%d', $field_number, $field_number;

    my $options = $field_options->[$i];
    $sort_cmd .= $options if $options;
    # key-specific override, e.g. "n"
  }

  # example of multi-column sort with mixed alphanuma and numeric keys:
  #   "-field" => [ "chrom", "pos" ],
  #   "-field-options" => [ "", "n" ],
  ...
  $sort_cmd .= ' | gzip' if $compress;
  # fun fact: multiple gzip streams can be concatenated together
  # and will decompress as a single unit
